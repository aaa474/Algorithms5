\paragraph{\textcolor{red}{Acknowledgment (remember to edit this).}} Written by \textbf{Aslam Azes (aaa474)}. I spent about ten hours in total doing research and such to figure out how to solve these to the best of my abilities. I can admit that I used websites like StackOverflow and Geeks4Geeks to help me on problems four and five as they were the ones I struggled with solving the most. \textbf{I affirm that I have acknowledged every external help during the preparation of this document.}
...
\paragraph{\textcolor{brown}{Solution.}}
Define M as $a$ Turning machine and make x an input. M' will be if the function M(x) halts.
M' will on input $S$ run M(x) and if M(x) halts, then we run $a$ brute force algorithm to continue or return the result given. If M(x) does not halt, then M' won't either
If the cover was deciable, then it would lead to $a$ contradiction as $a$ Turing machine with A that solves it and H(M') results in M(x) not flowing

The halting problem would be impossible in that case
...
\paragraph{\textcolor{brown}{Solution.}}
This partition problem is NP because the polynomial time of both parts' sum is equal to $W$ and it can be done in O(n) time.
Let $T$ be the target, and let's create $a$ partition problem set $S$' of n + 2 integers $S$' = $S$ U {2T, sum x in the set $S$ x}
The sum of the elements in \($S$^{\prime }\) is 2 sum x + 2T, and W = $T$.
Since the subset from b1 to bn equals $T$, $T$ is $a$ valid partition, and conversly

This reduction is in polynomial time
...
\paragraph{\textcolor{brown}{Solution.}}
Let's create $a$ set of verticies from v1 to vn and set $T$ = $344$. For each i to n, we add an edge from vi-1 -> vi that has $a$ weight of 0
We also add an edge from vi-1 -> vi with $a$ weight of mi, which is the sum
The total weight of this cycle that is created will be the sum of all mi
This would be in polynomial time
